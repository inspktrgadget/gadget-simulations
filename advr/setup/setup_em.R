...
om_dir <- "~/gadget/simulations/advr/op_mods/var_rec"
em_dir <- "~/gadget/simulations/advr"
...
age_prop <- c(1, 0.75, 0.5, 0.2, 0.1, 0.01)
...
set_em_up <- function(x, stock_std, stockname, om_dir, fishing_scenario,
  ...
  samples <-
    parallel::mclapply(1:sample_reps, function(y) {
      ...
      spr_comp_data <-
        spr_survey_samples %>%
        strip_age_length_data(length_samples = 200,
                              age_samples = i,
                              quiet = TRUE)
      aut_comp_data <-
        aut_survey_samples %>%
