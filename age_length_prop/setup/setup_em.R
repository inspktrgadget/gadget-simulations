...
om_dir <- "~/gadget/simulations/op_mods"
em_dir <- "~/gadget/simulations/age_length_prop"
...
length_prop <- c(1, 0.1, 0.01, 0.001, 0.0001)
age_prop <- c(0.1, 0.01)
...
init_sd <- c(1:10, rep(10, 50))


for (i in length_prop) {
    for (j in age_prop) {
        #-----------------------------------------
        # create a directory for the current level
        level_dir <- paste_path(em_dir, paste("length", i, "age", j, sep = "_"))
	    ...
	    cat("\n", "Writing model ", stockname, "-", fishing_scenario, "\n")
            ...
            init_cond <-
                normalparamfile(age = ages,
                                area = 1,
                                age.factor = make_switches(paste(stockname, "init.age.mult", sep = ".")),
                                area.factor = make_switches(paste(stockname, "init.area.mult", sep = ".")),
            ...
            est_mod <-
                ...
                update_model("stocks",
                             ...
                             beta = to_gadget_formula(quote(bbin *
                                                            bbin.mult))) %>%
                update_model("stocks",
