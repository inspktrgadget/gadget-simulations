...
om_dir <- "~/gadget/models/simulations/op_mods/var_rec/flatfish"
...
stock_m <- rep(0.1, (maxage - minage + 1))
...
init_data <-
    normalparamfile(age = seq(minage, maxage, 1),
                    area = 1,
                    age.factor = 1e4 * 
                        exp(-cumsum(rep(0.1, (maxage - minage + 1)))),
                    area.factor = 1,
...
twt_mod <- simulate_model(flatfish_twt_stock, twt_fleet, "two_way_trip")
fd_mod <- simulate_model(flatfish_fd_stock, fish_down_fleet, "fish_down")
fmsy_mod <- simulate_model(flatfish_fmsy_stock, flat_msy_fleet, "flat_msy")
