...
base_om_dir <- "~/gadget/models/simulations/op_mods/capelin"
make_scenario_stock <- function(scenario) {
    ...
    stock_rec <- 
        ...
        mutate(number = number * exp(rnorm(n(), 0, 0.04)))
...
capelin_twt_stock <- make_scenario_stock("two_way_trip")
capelin_fd_stock <- make_scenario_stock("fish_down")
capelin_fmsy_stock <- make_scenario_stock("flat_msy")
