...
model_dir <-"~/gadget/simulations/age_data_var_rec"
  ...
  lapply(models, function(i) {
    lapply(spp, function(j) {
      lapply(scenarios, function(k) {
        ...
        mod_check <- paste_path(i,j,k)
        mods2exclude <-
          c("dp_mltnom_log/cod/flat_msy",
            "dp_ss_dome/cod/flat_msy",
            "dr_mltnom_dome/flatfish/two_way_trip")
        if (mod_check %in% mods2exclude) {
          return(NULL)
        } else
        {
        em_path <- 
          paste_path(model_dir, i, j, k)
        om_path <- paste_path("~/gadget/simulations/op_mods", j, k)
