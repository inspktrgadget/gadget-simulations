...
om_dir <- "~/gadget/simulations/age_len_prop/op_mods/var_rec"
em_dir <- "~/gadget/simulations/age_len_prop"
...
age_samples <- c(10, 25, 50, 100, 200, 500, 1000)
len_samples <- c(10, 25, 50, 100, 200, 500, 1000)
...
set_em_up <- function(x, stock_std, stockname, om_dir, fishing_scenario,
  ...
  base_om_dir <- paste_path("~/gadget/simulations/op_mods",
  ...
  stock_data <- stock_std[[x]]
  sel_type <- "log"
  # obtain model values to guide parameter bounds later
  source(paste_path("~/gadget/simulations/op_mods",
  ...
  catch_samples <- 
    ...
    filter(year >= survey_st_year) %>%
    survey_gadget(length_groups = lengrps,
                  survey_suitability = rep(0.00001, length(lengrps)),
                  survey_sigma = catch_error,
                  variable = "number.consumed") %>%
    filter(number > 0)
  
  samples <-
    parallel::mclapply(1:sample_reps, function(y) {
      #-------------------------------
      # survey length_group added data
      spr_survey_samples <- filter(survey, step == 2)
      aut_survey_samples <- filter(survey, step == 3)
      spr_comp_data <-
        spr_survey_samples %>%
        strip_age_length_data(length_samples = i,
                              age_samples = j,
                              quiet = TRUE)
      aut_comp_data <-
        aut_survey_samples %>%
      ...
      source(paste_path(em_dir, "setup", "setup_params.R"),
             local = TRUE)
      # write out files to each sub_dir
      write_gadget_file(params, path = sub_dir)
...
for (i in len_samples) {
  for (j in age_samples) {
    if (j > i) {
      ...
      level_dir <- 
        paste_path(em_dir, paste("age", j, "len", i, sep = "_"))
