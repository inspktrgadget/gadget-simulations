...
path <- "~/gadget/models/simulations/age_len_prop/results"
...
mod_levels <- 
  expand.grid(age = c(10, 50, 100), length = c(10, 50, 100)) %>%
  filter(!(age > length)) %>%
  mutate(mod = paste("age", age, "len", length, sep = "_")) %>%
  pull(mod)
mod_labels <- vapply(mod_levels, function(x) {
  .simpleCap(gsub("_", " ", x))
}, character(1))
...
param_re_plot <- function(param, title) {
  ...
  g <- 
    ...
    facet_wrap(spp ~ scenario) + 
    geom_hline(yintercept = 0, linetype = "dashed") + 
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 60, hjust = 1),
          plot.title = element_text(size = 20, hjust = 0.05, vjust = 1.2),
          axis.title = element_text(size = 20)) + 
    ggtitle(title) +
    labs(color = "Composition Data Scenario") + 
    xlab("Composition Data Scenario") + ylab("Relative Error (%)")
  return(g)
}
...
comm_alpha_plot <- param_re_plot("comm_alpha", expression(italic(alpha["fleet"])))
comm_l50_plot <- param_re_plot("comm_l50", expression(italic("L"[50]["fleet"])))
...
dep_re_plot <- function(value, title) {
  ...
  g <- 
    ...
    theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
    ggtitle(title) +
    labs(color = "Age Data Scenario") + 
    xlab("Age Data Scenario") + ylab("Relative Error (%)")
  return(g)
}
